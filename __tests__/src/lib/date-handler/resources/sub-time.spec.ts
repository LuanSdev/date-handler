...
import { subTime } from '@/lib/date-handler/resources';
...
describe('Sub time', () => {
  ...
  it('Should throws if provided unit time is invalid.', () => {
    //eslint-disable-next-line
    //@ts-ignore
    const sut = () => subTime(new Date(), { timeUnit: 'invalid-time-unit' });

    expect(sut).toThrow(new Error('Invalid time unit.'));
  });

  it('Should sub correct time to provided date.', () => {
    type TOptions = {
      timeUnit: TTimeUnits;
      qtd: number;
      expect: number;
    ...
    options.forEach((option) => {
      ...
      const updatedDate = subTime(date, {
        timeUnit: option.timeUnit,
        qtd: option.qtd,
      });

      const diff = date.getTime() - updatedDate.getTime();

      expect(diff).toBe(option.expect);
    });
  });

  it('Should sub 1 of provided time unit if no qtd is provided.', () => {
    const date = new Date();
    const updatedDate = subTime(date, { timeUnit: 'day' });
